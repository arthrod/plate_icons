...
const TokenProto = ObjectCreate(null, { loc: {
	get() {
...
var require_eslint_visitor_keys = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	let types_1 = require_dist$1(), assert_1 = require_assert(), definition_1 = require_definition(), ID_1 = require_ID(), Reference_1 = require_Reference(), variable_1 = require_variable(), ScopeType_1 = require_ScopeType(), ImplicitLibVariable_1 = require_ImplicitLibVariable();
	...
	let types_1 = require_dist$1(), assert_1 = require_assert(), ImplicitGlobalVariableDefinition_1 = require_ImplicitGlobalVariableDefinition(), variable_1 = require_variable(), ScopeBase_1 = require_ScopeBase(), ScopeType_1 = require_ScopeType();
	...
	let assert_1 = require_assert(), ScopeBase_1 = require_ScopeBase(), ScopeType_1 = require_ScopeType();
...
function walkTSImportType(node, visitors) {
	...
	ancestors.unshift(node), walkNode(node.source, visitors), walkNode(node.options, visitors), walkNode(node.qualifier, visitors), walkNode(node.typeArguments, visitors), ancestors.shift(), exit !== null && exit(node);
}
...
const { matches: esqueryMatches, parse: esqueryParse } = (/* @__PURE__ */ __toESM(require_esquery_min(), 1)).default, ESQUERY_OPTIONS = {
	nodeTypeKey: "type",
	visitorKeys: keys_default,
	fallback: (node) => ObjectKeys(node).filter(filterKey),
...
const compilingLeafVisitor = [], compilingNonLeafVisitor = [];
...
const activeLeafVisitorTypeIds = [], activeNonLeafVisitorTypeIds = [];
for (let i = 27; i !== 0; i--) activeLeafVisitorTypeIds.push(0);
for (let i = 138; i !== 0; i--) activeNonLeafVisitorTypeIds.push(0);
activeLeafVisitorTypeIds.length = 0, activeNonLeafVisitorTypeIds.length = 0;
...
function addVisitorToCompiled(visitor) {
	...
	if (keysLen !== 0) {
		...
		for (let i = 0; i < keysLen; i++) {
			...
			if (typeId !== void 0) {
				visitProp.specificity |= 1, addVisitFn(typeId, visitProp, isExit);
				continue;
			}
			if (name !== "*") {
				...
				if (typeIds !== null) {
					for (let i$1 = 0, len = typeIds.length; i$1 < len; i$1++) addVisitFn(typeIds[i$1], visitProp, isExit);
					continue;
				}
			}
...
function addVisitFn(typeId, visitProp, isExit) {
...
function finalizeCompiledVisitor() {
	...
	return visitPropsCacheNextIndex = 0, activeLeafVisitorTypeIds.length = 0, activeNonLeafVisitorTypeIds.length = 0, hasActiveVisitors = !1, !0;
}
const visitFns = [];
...
function lintFile(filePath$1, bufferId, buffer$1, ruleIds, optionsIds, settingsJSON$1, globalsJSON$1) {
	try {
		return lintFileImpl(filePath$1, bufferId, buffer$1, ruleIds, optionsIds, settingsJSON$1, globalsJSON$1), diagnostics.length === 0 ? null : JSONStringify({ Success: diagnostics });
	} catch (err) {
		return JSONStringify({ Failure: getErrorMessage(err) });
	} finally {
